...
class PPOAgent:
    ...
    def run_trajectory(self):
        ...
        for t in range(self.nsteps):
            ...
            # a = tf.clip_by_value(a, clip_value_min=self.min_a, clip_value_max=self.max_a)

            o2, r, d, _ = env.step(a.numpy().reshape(-1, ))
            r = r / self.reward_scale  # apply reward scaling for stability
            ep_ret += r
            ep_len += 1

            # save and log: scaled observation, unscaled action, reward, value, logprob of suggested action
            buf.store(o, a.numpy(), r, v_t.numpy().reshape(-1, ), logp_t.numpy().reshape(-1, ))

            # Update obs
            o = o2

            terminal = d or (ep_len == self.max_episode_len)
            if terminal or (t == self.nsteps - 1):
                if not (terminal):
                    print('Warning: trajectory cut off by epoch at %d steps.' % ep_len)

                # if trajectory didn't reach terminal state, bootstrap value target
                o = o / obs_scaling_constant
                _, _, v_term = self.sample_action(o.reshape(1, -1) / obs_scaling_constant)
    ...
    def train(self,
        ...
        self.nsteps = nsteps
        vf_clip_param = self.reward_scale
        ...
        for update in range(updates):
            ...
            num_batches = max(int(obs.shape[0] / minibatch_size), 1)
            ...
            for epoch in range(epochs):
                ...
                for i in range(num_batches):
                    ...
                    with tf.GradientTape(persistent=True) as tape:
                        ...
                        # value fn loss
                        vf_loss = tf.math.square(v - ret_batch)
                        # vf_loss_clipped = tf.clip_by_value(vf_loss, 0, vf_clip_param)
                        vf_loss = tf.math.reduce_mean(vf_loss)
                    ...
                    if joint_train:
                        ...
                        if np.abs(approx_kl) > target_kl:
                            ...
                            pi_optimizer.apply_gradients(zip(grads, self.model.trainable_variables))
                        print(
                            "  Epoch: {}, iteration: {}, kl_div: {}, entropy: {}, pg_loss: {}, vf_loss: {}, loss: {}".format(
                                epoch, i, approx_kl, approx_ent, pg_loss, vf_loss, loss))
    ...
    def output(self, model_path):
        ...
        for t in range(self.env.planning_periods):
            ...
            action, _, _, _, _ = self.model(state.reshape(1, -1))
