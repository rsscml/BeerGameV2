...
class InvManagementMasterEnv(gym.Env):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.dist < 5:
            reward_scale = self.demand_dist.stats(**self.dist_param, moments='m')
        else:
            reward_scale = np.mean(self.demand_dist)  # user specified demand

        self.reward_scale = reward_scale * self.p
